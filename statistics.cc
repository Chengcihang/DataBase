...
//注册对统计信息的更改
RC StatisticsManager::Register (const char *psKey,
...
//打印与特定统计信息有关的信息
RC StatisticsManager::Print(const char *psKey) {
    if (psKey==NULL)
        return STAT_INVALID_ARGS;
...
int *StatisticsManager::Get(const char *psKey) {
    int i, iCount;
    Statistic *pStat = NULL;
...
//重置特定的统计信息
RC StatisticsManager::Reset(const char *psKey) {
    int i, iCount;
    Statistic *pStat = NULL;
