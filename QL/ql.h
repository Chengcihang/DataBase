...
public:
    ...
    RC Insert  (const char *relName,     // 需插入的关系表名
                int   nValues,                   // 要插入的属性个数
                const Value values[]);           // 属性数组
...
private:
    // 初始化各参数 准备执行下一条命令 Resets the class variables for the next query command
    RC Reset();
    // 检查查询操作的属性是否合法 Checks whether an attribute is valid for a query command
    bool IsValidAttr(const RelAttr attr);
    // 判断操作涉及的关系表名存在于数据库中且关系表名不重复 Parses the relations, making sure they're in the database, and there aren't duplicates
    RC ParseRelNoDup(int nRelations, const char * const relations[]);

    // 解析select操作涉及的属性 判断是否合法 Parses the select attributes, making sure they're valid
    RC ParseSelectAttrs(int nSelAttrs, const RelAttr selAttrs[]);
    //todo:Gets the attribute attrCatEntry associated with a specific attribute
    RC GetAttrCatEntry(const RelAttr attr, AttrCatEntry *&entry);
    //todo:Get the attribute attrEntries position associated with a specific attribute
    RC GetAttrCatEntryPos(const RelAttr attr, int &index);
    // 解析操作中的条件语句 判断是否合法 Parses the conditions to make sure that they're valid
    ...
    // 肩检查update操作涉及的属性是否合法 Check that the update attributes are valid
    ...
    // 执行select操作 参数是头节点 Runs select given the top node
    RC RunSelect(QL_Node *topNode);
    // 将一系列值插入关系表 Inserts a set of values into a relation
    RC InsertIntoRelation(const char *relName, int tupleLength, int nValues, const Value values[]);
    // 向关系的索引中插入新纪录 Inserts a new record in all the indices belonging to that relation
    ...
    // 一次操作后清除节点 Cleans up the nodes after a run
    ...
    // 为生成查询树初始化第一个节点 Set up the first node in creating the query tree
    RC SetUpFirstNode(QL_Node *&topNode);
    // （不使用查询优化器）生成整个查询树并返回头节点 Sets up the entire query tree and returns the top node in topNode
    ...
    // 存储关系表名向该关系表在relEntries中下标的映射关系的map     //todo 这个relEntries是个存储所有关系表的数组吗
    std::map<std::string, int> relToInt;
    // 存储属性名向含有该属性名所有关系表名集合的映射关系的map maps from attribute name to a set of its relation names
    std::map<std::string, std::set<std::string> > attrToRel;
    // 存储关系表名向该关系中第一个关系下标的映射的map maps from relation name to the index of its first relation //todo 存疑
    ...
    // 属性、关系、条件数量 number of attributes, relations and conditions
    int nAttrs;
    int nRels;
    int nConds;
    // 是否有更新操作（决定是否使用索引）whether it's an update or not (whether to use indices or not)
    bool isUpdate;

    // 指向条件列表的指针 pointer to the condition list
...
// QL模块输出错误函数 Print-error funtion
