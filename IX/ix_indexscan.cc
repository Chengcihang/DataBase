...
RC IX_IndexScan::GetNextEntry (RID &rid) {
    RC rc = 0;
    //如果扫描结束，返回IX_EOF
    if(scanIsEnd == true && scanIsOpen == true)
        return (IX_EOF);
    //如果最后一个值已经找到页返回IX_EOF
    if(lastValueIsFound == true)
        return (IX_EOF);

    //如果扫描器并未打开，则返回IX_INVALIDSCAN
    if(scanIsEnd == true || scanIsOpen == false)
        return (IX_INVALIDSCAN);
    ...
    while(hasFoundFlag) {
        //扫描的第一次迭代
        if(scanIsEnd == false && scanIsOpen == true && scanIsStart == false){
            //开始扫描，检索第一个项
...
//            return (rc);
        ...
        else if(firstValueIsFound == true){
...
RC IX_IndexScan::setRID(bool setCurrent){
    ...
    if(isReachedEndOfIndex && setCurrent == false){
        RID rid1(-1,-1);
        nextRID = rid1;
        return (0);
    }
